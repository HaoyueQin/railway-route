...
- 提供命令行输出和本地 Web GUI（前端为独立 `web/` 静态文件：`index.html` + `styles.css` + `app.js`）；
- 全部下拉框为纯 JS 自定义玻璃态控件，不依赖原生 `<select>` 面板（无白底兼容问题）；
- 结果区支持六维排序（综合/耗时/里程/出发/到达/换乘次数）与换乘类型/城市站名筛选，切换不丢状态；
- 路线卡片紧凑节点+箭头流式布局，换乘站显示双时刻，点击展开查看分段时刻表；
- **60 个确定性 `unittest` 测试**，核心运行代码仅使用 Python 标准库。
...
以下为 2026-07-31 当前数据和代码在开发笔记本上的代表性单次运行结果：

| 查询 | 首条候选 | 结果数 | 加载 | 搜索 |
|---|---|---|---|---:|
| 北京南→上海虹桥 (exact, fast) | G37, 4h43m, 直达 | 12 | 0.6s | 0.9s |
| 北京南→上海虹桥 (exact, balanced) | G37, 4h43m, 直达 | 29 | 0.6s | 1.9s（完整） |
| 北京→上海 (fuzzy, balanced) | G3, 4h41m, 北京南→上海 | 257 | 0.6s | 2.8s |
| 哈尔滨→昆明 (fuzzy, balanced) | G3492/G3493→D939, 27h44m, 郑州东换乘 | 141 | 0.6s | 5.5s（完整） |
| 延安→深圳北 (fuzzy, complete) | K5445→…→G419/G422 等 | 659 | 0.6s | 15s（完整） |

算法：轮次化多标签 CSA（RAPTOR 风格）＋按站分桶连接堆归并迭代＋目标导向剪枝
（多源反向 Dijkstra 距离/时间下界＋多标签预扫描锁定"每换乘级别最短耗时"基准）。
`complete` 档完整跑完所有换乘轮（旧版 8.1s 截断从未完整完成）。正确性由
`tools/ab_verify.py` 与全量扫描参考实现逐条对比验证（7 个代表性查询 miss=0 / extra=0）。

2026-07 增强：
- **车次全程时刻表**：方案详情显示始发终到站 + 上站上一站→下站下一站区间时刻，其余折叠；
- **动效**：结果入场 stagger、下拉/手风琴动画（GPU 合成）、三角可开/关切换；
- **每端独立匹配**：半精确半模糊（如 乌鲁木齐全部站 → 北京西单站），GUI/CLI/API 三端支持；
- **匹配规则贴近生活**：城市名→全市；区→所属市（怀柔→北京）；县/镇班次充足→单站（新县/燕郊）；班次稀疏→扩散（广阳→廊坊）；
- **剪枝动态化**：transfer_at 约束时放宽绕路比与耗时 slack（经郑州 balanced 139 条 / complete 354 条）；
- **查询缓存**：内存 LRU + SQLite 持久化，重复查询 0.02s 命中（约 250 倍）；
- 内存占用约 170MB（RSS），远低于 1G 约束。
...
│   ├── main.py                  # 评分、CLI、Web GUI/API（服务 web/ 静态文件）
│   ├── search.py                # 旧搜索实现，当前入口未使用
│   ├── score.py                 # 旧评分实现，当前入口未使用
│   └── transfer.py              # 旧换乘实现，当前入口未使用
├── web/
│   ├── index.html               # GUI 页面骨架（结构）
│   ├── styles.css               # 玻璃态样式（布局/下拉/路线流/筛选条）
│   └── app.js                   # GUI 行为（搜索/渲染/排序筛选/自定义下拉）
└── tests/
    ...
    └── test_web_html.py         # 前端结构契约 + 静态服务测试
```
...
已修复。前端所有下拉框（表单区 + 结果区排序筛选）均由 `app.js` 的 `buildDropdown()` 纯 JS 构建，不再依赖浏览器原生 `<select>` 下拉面板，样式统一为半透明玻璃面板，不存在原生白底兼容问题。
...
注意：部分 Windows 机器的 Hyper-V 会把一段端口（如 7954-8053）列入排除范围，8000 可能永远无法绑定。可用 `netsh interface ipv4 show excludedportrange protocol=tcp` 检查，换一个空闲端口即可（如 8765）。

## 近期路线

1. 补充车次完整经停站数据到展开时刻表（当前仅显示上下车站）；
2. 基于车站坐标估算异站换乘时间；
3. 完善评分配置（用户可调权重）；
4. 端口占用友好处理与桌面打包；
5. 开行日期过滤。

...
> ## ⚠️ 本分支（master-v3）为实验分支，已废弃
>
> 本分支仅用于 **RAPTOR 路线级重构 + rayon 多核并行** 的一次性实验（2026-08）：
> 实验结论为 **路线级扫描可减半（-50%）但对拍结果与连接级不完全一致（M3 210 组合仅 16 通过），
> 且多核并行在本机为负收益（120s vs 串行 111s，根因：Arc 原子计数竞争 + 消费级内存带宽瓶颈）**。
> 因此该路线不采用，**生产代码请使用 `master-v2` 分支**（全量对拍 0 失败）。
> 本分支保留仅为存档，不再维护，勿基于此分支开发。
